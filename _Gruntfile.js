...
module.exports = function(grunt) {'use strict';
  ...
  grunt.initConfig({
    ...
    concat: {
      ...
      docs: {
        ...
        src: [],
        dest: 'scripts/<%= filename %>.tpls.js'
      },
      'docs-demo': {
        src: [
          'src/scripts/docs/conf/config.js',
          'src/scripts/docs/libs/app.js',
          'src/scripts/docs/libs/*.js',
          'src/scripts/docs/models/*.js',
          'src/scripts/docs/public/app.js',
          'src/scripts/docs/public/*.js',
          'src/scripts/docs/pages/*.js',
          'src/scripts/docs/pages/*/*.js'
        ],
        dest: 'scripts/<%= filename %>.docs.js'
      }
    },
  ...
  grunt.registerTask('script-custom', 'Create custom scripts files.', function(a) {
    ...
    // Develop mode
    if (a === 'dev') {
      grunt.config('concat.docs.src', grunt.config('concat.docs.src').concat(srcFiles).concat(tpljsFiles));
      grunt.task.run(['concat:docs']);
    }
    // Custom mode
    else {
      ...
      grunt.task.run(['concat:dist', 'concat:distTpls', 'uglify:dist', 'uglify:distTpls']);
    }
  });

  grunt.registerTask('test',      []);
  grunt.registerTask('dev',       ['clean', 'copy:docs', 'less:docs', 'script-custom:dev', 'concat:docs-demo', 'jade', 'watch']);
  grunt.registerTask('release',   ['clean', 'copy:docs', 'less:docs', 'script-custom:dev', 'concat:docs-demo', 'uglify:docs', 'hashmap', 'jade']);
  grunt.registerTask('build', 'Create custom files.', function() {
    var args = this.args.length ? ':' + this.args.join(':') : '';
    grunt.task.run(['clean', 'html2js', 'style-custom' + args, 'script-custom' + args, 'clean:build']);
  });
